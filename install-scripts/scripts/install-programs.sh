...
read x
# Should change these so there is just an extra option for dev tools. That way we aren't double listing.
# Add:  termdown (pacman), potato/ordne (yay1)
if [ $x == 1 ]; then
  echo "Full install, including development tools"
  echo -e "\nStarting pacman installs:"
  sudo pacman -Su qutebrowser pass rofi ranger termdown python-pywal r bluez trash-cli bluez-utils pulseaudio pulseaudio-bluetooth jupyterlab python syncthing firefox chromium libreoffice-fresh gimp pdfarranger inkscape transmission-gtk openvpn sshfs python-adblock pavucontrol scrot redshift atril ttf_apple_emoji bash-completion thunderbird clipit speedtest-cli gvim evince
  echo -e "\nSwitching to AUR installs via 'yay' AUR helper:"
  yay -Su franz-bin potato rstudio-desktop-bin spotify notion-app chromium-widevine zoom nordvpn-bin pulseffects-legacy
  ...
  echo -e "\nSwitching to AUR installs via 'yay' AUR helper:"
  yay -Su franz-bin potato spotify notion-app chromium-widevine zoom nordvpn-bin pulseffects-legacy
